...
def _minimum_scaled_jacobian(xy, element_rows, tolerance):
    """Return the worst scaled Jacobian, or ``None`` for invalid geometry."""
...
def _strip_edges(elements):
    """Return one directed representative of every topological strip edge."""
...
def _validate_generated_strip(
    ...
    """Validate geometry and non-local edge intersections for one new strip."""
    ...
    *,
    ...
    for sweep in range(50):
        ...
        for position in positions:
            if fixed[position]:
                continue
            if not closed and position in (0, angles.size - 1):
                continue
...
def _to_circle(
    ...
    def validate_target_circle(candidate_xy):
    ...
    validate_target_circle(target_xy)
    ...
    try:
        ...
        validate_target_circle(smoothed_target_xy)
